...
export const MainLive = DiscordBotService.Default.pipe(
  // Discord Bot depends on Gateway, MessageHandler, InteractionHandler, and SlashCommands
  Layer.provide(DiscordGatewayService.Default),
  Layer.provide(MessageHandlerService.Default),
  Layer.provide(InteractionHandlerService.Default),
  Layer.provide(SlashCommandService.Default),
  ...
  // SlashCommands and InteractionHandler depend on Claude CLI
  Layer.provide(ClaudeCliService.Default),
  // Everything depends on Config
  Layer.provide(ConfigService.Default),
  // AI Service needs HTTP client
  Layer.provide(FetchHttpClientLayer),
  // Claude CLI needs CommandExecutor and FileSystem
